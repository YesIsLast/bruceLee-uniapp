<template>
	<view class="uni-calendar" @touchmove.stop.prevent="clean">
		...
		<view v-if="insert || show" class="uni-calendar__content" :style="borderRadius" :class="{'uni-calendar--fixed':!insert,'uni-calendar--ani-show':aniMaskShow}">
			...
			<view class="uni-calendar__header" v-if="selecteMonth">
			...
			<view class="" style="padding: 30rpx;" v-if="!selecteMonth">
				<text style="font-size: 35rpx;color: #363636;font-weight: 600;padding-right: 20rpx;">{{calendarTitle}}</text>
				<text style="font-size: 30rpx; color: #999999;">({{( nowDate.month||'-') +'月'}})</text>
			</view>
...
<script>
	 ...
	 * 	@value true 弹窗模式
	 * 	@value false 插入模式
	 * @property {Array} selected 打点，clockStatus 是否打卡，期待格式[{date: '2019-06-27',clockStatus:false, info: '签到', data: { custom: '自定义信息', name: '自定义消息头',xxx:xxx... }}]
